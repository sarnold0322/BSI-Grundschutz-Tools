...
<#
.SYNOPSIS
Führt den Befehl "computerinfo" aus und prüft, ob SecureBoot darin vorkommt und gibt das Ergebnis als JSON-Objekt aus.
...
try {
    $output = (Get-ComputerInfo).DeviceGuardAvailableSecurityProperties

    if ($output -contains "SecureBoot") {
        $result = 1
    } else {
        $result = 0
    }
